class Chatroom{
    ...
    async addChat(mess){//asinhroni metod, on radi nezavisno i paralelno u odnosu na ostatak koda.
        //Ova metoda prima kao parametar poruku i tu poruku smesta u bazu. Mess je u stvari 
        ...
        let docChat={
            ...
            room: this.room,
            created_at: firebase.firestore.Timestamp.fromDate(date)//u zagrdai je gore formiran datum
        };
        ...
        .onSnapshot(snapshot=>{//onSnapshot je metod. 
            snapshot.docChanges().forEach(change=>{//docChanges je niz promena, change je tekuca promena
                ...
                if(change.type==='added'){
                    //uraditi update
                    callback(change.doc.data());//sa callback cuvamo podatke o promeni 
