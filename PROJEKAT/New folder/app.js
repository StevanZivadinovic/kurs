...
let formaPeriod=document.querySelector('#formaPeriod');
let btnPeriod=document.querySelector('#btnPeriod');
let startdate=formaPeriod.start_date.value;
let dueDate=formaPeriod.due_date.value;
formaPeriod.addEventListener('submit',event=>{
...
let colorPicker=document.querySelector('#colorPiker');
let btn5=document.querySelector('#btnColor');
let boja=document.querySelector('#color').value;

btn5.addEventListener("click", (e) => {
    e.preventDefault();
    let trenutna=colorPiker.color.value;
    setTimeout(() => {
        document.body.style.backgroundColor = colorPiker.color.value;//colorInput.value;
    }, 500);
    localStorage.setItem('bojaLS',trenutna );
});

if(localStorage.bojaLS){
    document.body.setAttribute("style", `background-color: ${localStorage.bojaLS}`);
}
else {
