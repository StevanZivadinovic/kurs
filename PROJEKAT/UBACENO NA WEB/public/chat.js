export class Chatroom{
    ...
    getChets(callback){
        this.unsub = this.chats
                    ...
                    .orderBy('created_at')//redja poruke od najstarije do najmladje
                    .onSnapshot( snapshot => {
                        snapshot.docChanges().forEach( change => {
                            //Ako je poruka dodata u bazu onda raditi update
                            if( change.type === 'added' ){
                                //uradi update
                                callback(change.doc.data());//callback funkcija je isto kao eventListener
                                //ocekuje nesto. Ovde callback ocekuje promenu tipa added, kad se desi dodavanje
                                //ona se aktivira
                            }
                        });
                    });
    ...
    updateUsername(uu){
        ...
        this.username = uu;//ovde setujemo vrednost
        localStorage.setItem('usernameLS', uu);//ovo radimo da bi uvek kad nanovo udjemo mogli da
        //vidimo koji korisnik se zadnji ulogovao, tj uneo ime u plje username
    }
